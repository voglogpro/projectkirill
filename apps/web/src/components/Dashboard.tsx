import { ArrowRight, Bot, CheckCircle2, ChevronRight, Circle, Eye, FileText, GitBranch, Globe, HelpCircle, LayoutDashboard, LogOut, MessageSquare, Paintbrush, Plus, Rocket, Settings, Smartphone, Users } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { fetchLeads, getCurrentUser, hasSession, listRemoteProjects, logout, renameRemoteProject } from "../api";
import type { DashboardSection, Lead, ProjectState } from "../types";
import { PhonePreview } from "./PhonePreview";

interface Props { project: ProjectState; onProjectChange: (project: ProjectState) => void; onEdit: () => void; onEditFlow: () => void; onPreview: () => void; onLaunch: () => void; onHome: () => void; onNewProject: () => void; onOpenProject: (id: string) => Promise<void>; onGuide: () => void; onMessage: (message: string) => void }
const mobileSections: DashboardSection[] = ["overview", "flow", "leads", "design"];
...
export function Dashboard(props: Props) {
  const { project, onProjectChange, onEdit, onEditFlow, onPreview, onLaunch, onHome, onNewProject, onOpenProject, onGuide, onMessage } = props;
  ...
  <div className="mobile-workspace-bar">
    ...
    <button onClick={() => { logout(); onHome(); }} aria-label="Выйти из аккаунта"><LogOut /><span>Выйти</span></button>
  </div>
  <main className="dashboard"><header><div><span className="muted">{project.name} /</span><h1>{title}</h1></div>{section !== "settings" && <button className="primary-button" onClick={primaryAction}>{isLive && !hasPendingChanges ? <Eye /> : <Rocket />}{primaryLabel}</button>}</header>
    {section === "overview" && <Overview project={project} page={page} onEditFlow={onEditFlow} completed={completed} progress={progress} statusLabel={statusLabel} hasForm={hasForm} onEdit={onEdit} onPreview={onPreview} onLaunch={onLaunch} onHelp={() => setSection("help")} />}
    {section === "bot" && <SimplePanel icon={<Bot />} title={botConnected && project.botUsername ? `@${project.botUsername} подключён` : connectionNeedsAttention ? "Telegram-бот требует переподключения" : "Бот ещё не подключён"} text={botConnected ? "Кнопка меню ведёт в актуальную опубликованную версию Mini App." : connectionNeedsAttention ? "Подключение Telegram не завершено или было отозвано. Вставьте актуальный токен — мастер безопасно восстановит Menu Button и webhook." : "Создайте бота через @BotFather. Мастер проверит токен, зашифрует его и настроит Menu Button автоматически."} action={connectionNeedsAttention ? "Переподключить бота" : project.botUsername ? !isLive ? "Продлить доступ" : hasPendingChanges ? "Опубликовать изменения" : "Открыть бота в Telegram" : "Подключить бота"} onAction={project.botUsername && isLive && !hasPendingChanges ? () => window.open(`https://t.me/${project.botUsername}`, "_blank", "noopener,noreferrer") : onLaunch}><button className="outline-button" onClick={onEditFlow}>Собрать сценарий бота</button>{project.miniAppUrl && <a className="outline-button" href={project.miniAppUrl} target="_blank" rel="noreferrer">Проверить Mini App в браузере</a>}{project.miniAppUrl && <a className="outline-button" href={siteUrlOf(project.miniAppUrl)} target="_blank" rel="noreferrer"><Globe />Открыть сайт</a>}</SimplePanel>}
    {section === "leads" && <LeadsPanel leads={leads} loading={loadingLeads} onTest={onPreview} />}
    {section === "settings" && <section className="panel settings-panel"><span>ПРОЕКТ</span><h2>Настройки проекта</h2><label><span>Название</span><input name="project-name" value={name} maxLength={120} onChange={(event) => setName(event.target.value)} /></label><button className="primary-button" disabled={name.trim().length === 0 || name === project.name} onClick={() => { const nextName = name.trim(); if (!hasSession()) { onProjectChange({ ...project, name: nextName }); onMessage("Название сохранено на этом устройстве"); return; } void renameRemoteProject(project.id, nextName).then(() => { onProjectChange({ ...project, name: nextName }); onMessage("Название проекта сохранено"); }).catch((reason) => onMessage(reason instanceof Error ? reason.message : "Не удалось сохранить")); }}>Сохранить</button><div className="settings-meta"><span>Статус: <b>{statusLabel}</b></span><span>Страниц: <b>{project.pages.length}</b></span></div></section>}
  ...
  <nav className="mobile-nav">{nav.filter((item) => mobileSections.includes(item.id)).map(({ id, label, icon: Icon }) => <button key={id} className={section === id ? "active" : ""} onClick={() => openSection(id)}><Icon /><span>{id === "settings" ? "Настр." : label}</span></button>)}</nav>
  </div>;
}
