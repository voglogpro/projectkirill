...
function form(publicId: string, pageId: string, props: Record<string, unknown>): HTMLFormElement {
  ...
  for (const field of fields) {
    ...
    if (field.kind === "select") { const select = element("select"); select.required = field.required ?? false; const placeholder = element("option"); placeholder.value = ""; placeholder.textContent = "Выберите"; select.append(placeholder); for (const option of field.options ?? []) { const item = element("option"); item.value = option.value; item.textContent = option.label; select.append(item); } select.addEventListener("change", () => { values[field.id] = select.value; }); label.append(select); }
    else if (field.kind === "checkbox") { const input = element("input"); input.type = "checkbox"; input.required = field.required ?? false; input.addEventListener("change", () => { values[field.id] = input.checked; }); label.append(input); }
    else { const input = element("input"); input.type = field.kind === "email" ? "email" : field.kind === "phone" ? "tel" : "text"; input.required = field.required ?? false; input.addEventListener("input", () => { values[field.id] = input.value; }); label.append(input); }
    node.append(label);
  }
