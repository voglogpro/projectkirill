...
function renderBlock(block: AppBlock, publicId: string, page: AppPage, navigate: (id: string) => void): HTMLElement {
  ...
  if (block.type === "media") { const node = element("img", "media"); node.src = String(p.url ?? ""); node.alt = String(p.alt ?? ""); node.loading = "lazy"; const ratio = String(p.aspectRatio ?? "auto"); if (ratio !== "auto") node.style.aspectRatio = ratio.replace(":", "/"); node.addEventListener("error", () => { node.classList.add("media-error"); node.alt = "Изображение не загрузилось"; }); return node; }
  if (block.type === "button") return actionButton(p, navigate);
  if (block.type === "product") return product(p, navigate);
...
function actionButton(props: Record<string, unknown>, navigate: (id: string) => void): HTMLButtonElement {
  ...
  button.addEventListener("click", () => { const haptic = toHaptic(props.haptic); if (haptic !== undefined) telegram.haptic(haptic); executeAction(action, navigate); });
  return button;
}

function product(props: Record<string, unknown>, navigate: (id: string) => void): HTMLElement {
  const price = props.price as { amountMinor?: number; currency?: string } | undefined;
  const cta = props.cta as { label?: string; action?: { kind?: string; url?: string; pageId?: string } } | undefined;
  ...
  const row = element("div"); const value = element("b"); value.textContent = formatMoney(price?.amountMinor ?? 0, price?.currency ?? "RUB"); const button = element("button"); button.textContent = cta?.label ?? "Выбрать"; button.addEventListener("click", () => { telegram.haptic(); executeAction(cta?.action, navigate); }); row.append(value, button); body.append(row); article.append(body); return article;
}
...
function form(publicId: string, pageId: string, props: Record<string, unknown>): HTMLFormElement {
  ...
  for (const field of fields) {
    ...
    else { const input = field.kind === "text" && (field as { multiline?: boolean }).multiline ? element("textarea") : element("input"); input.name = field.id; if (input instanceof HTMLInputElement) input.type = field.kind === "email" ? "email" : field.kind === "phone" ? "tel" : "text"; input.autocomplete = field.kind === "email" ? "email" : field.kind === "phone" ? "tel" : field.id.toLowerCase().includes("name") ? "name" : "off"; input.required = field.required ?? false; const maxLength = (field as { maxLength?: number }).maxLength; if (maxLength !== undefined) input.maxLength = maxLength; input.addEventListener("input", () => { values[field.id] = input.value; }); label.append(input); }
    node.append(label);
  }
...
function executeAction(action: { kind?: string; url?: string; pageId?: string } | undefined, navigate: (id: string) => void) { if (action?.kind === "page" && action.pageId) navigate(action.pageId); else if (action?.url) telegram.open(action.url); }
function toHaptic(value: unknown): "light" | "medium" | "heavy" | undefined { if (value === "none") return undefined; return value === "medium" || value === "heavy" ? value : "light"; }
