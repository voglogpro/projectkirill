...
function form(publicId: string, pageId: string, props: Record<string, unknown>): HTMLFormElement {
  ...
  for (const field of fields) {
    ...
    else { const input = element("input"); input.name = field.id; input.type = field.kind === "email" ? "email" : field.kind === "phone" ? "tel" : "text"; input.autocomplete = field.kind === "email" ? "email" : field.kind === "phone" ? "tel" : field.id.toLowerCase().includes("name") ? "name" : "off"; input.required = field.required ?? false; input.addEventListener("input", () => { values[field.id] = input.value; }); label.append(input); }
    node.append(label);
  }
